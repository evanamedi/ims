...
from api_v3.models import db, Product
...
class ProductQueryResource(Resource):
    def __init__(self):
        self.model = Product
        self.operations = {
            'all': self.get_all,
    ...
    def get_all(self, field, value):
        return {'data': [self.row2dict(row) for row in db.session.query(self.model).all()], 'message':'Products Retrieved Successful'}, 200
